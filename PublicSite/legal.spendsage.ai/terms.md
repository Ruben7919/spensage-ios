# Términos de Uso de SpendSage

Última actualización: 2026-04-05

## 1. Aceptación

Al descargar, acceder o utilizar SpendSage, usted acepta estos Términos de Uso. Si no está de acuerdo con ellos, no debe utilizar la aplicación.

Para fines de identificación del operador actual de la aplicación, SpendSage es operada por:

- `Ruben Lazaro`
- RUC: `0924829179001`
- Domicilio declarado: `Samanes 4 Mz 408 Villa 130112`

## 2. Naturaleza del servicio

SpendSage es una aplicación de apoyo para registro financiero personal, organización presupuestaria, lectura asistida de recibos, visualización de métricas y acompañamiento conductual. SpendSage:

- no es un banco;
- no es una entidad de pago;
- no es un asesor financiero, fiscal, contable o legal;
- no garantiza ahorro, rentabilidad, resultados financieros concretos ni exactitud absoluta de los datos procesados.

Toda decisión financiera sigue siendo responsabilidad exclusiva del usuario.

## 3. Elegibilidad y uso permitido

Usted declara que tiene capacidad legal suficiente para aceptar estos términos y usar la aplicación conforme a la ley aplicable.

Usted se compromete a utilizar SpendSage únicamente para fines lícitos y personales o internos, salvo autorización expresa distinta.

## 4. Cuenta y acceso

Algunas versiones o superficies de SpendSage pueden requerir o promover el uso de una cuenta. Otras funciones pueden operar localmente en el dispositivo.

Usted es responsable de:

- la exactitud de la información que introduce;
- la seguridad de sus credenciales y del dispositivo utilizado;
- cualquier actividad realizada desde su cuenta o su dispositivo, salvo que la ley disponga otra cosa.

## 5. Beta, TestFlight y funciones en evolución

La aplicación puede distribuirse en versiones beta, internas o de preproducción, incluyendo TestFlight. En esas versiones:

- funciones, flujos, precios, planes y pantallas pueden cambiar sin previo aviso;
- determinadas superficies pueden ser demostrativas, preparatorias o parciales;
- la estabilidad y disponibilidad pueden ser inferiores a una versión final;
- ciertos componentes mostrados en la interfaz pueden no estar plenamente activados en backend o facturación.

El uso de builds beta es bajo su propio criterio y riesgo.

## 6. Licencia limitada

Sujeto a su cumplimiento de estos términos, se le concede una licencia limitada, revocable, no exclusiva, no sublicenciable e intransferible para usar SpendSage en dispositivos que usted posea o controle, conforme a las reglas de Apple y a la ley aplicable.

No se le concede ningún derecho de propiedad intelectual sobre la aplicación, salvo el derecho limitado de uso aquí descrito.

## 7. Restricciones

Usted no debe, salvo en la medida permitida por la ley:

- copiar, modificar, traducir, adaptar o crear obras derivadas de la aplicación;
- descompilar, hacer ingeniería inversa o intentar obtener código fuente;
- vulnerar medidas de seguridad, límites técnicos o mecanismos de acceso;
- usar la aplicación para actividades fraudulentas, abusivas, ilícitas o engañosas;
- introducir contenido malicioso, automatización abusiva o tráfico que degrade el servicio;
- usar SpendSage para explotar o perfilar a terceros sin base legal adecuada.

## 8. Datos, contenido y exactitud

Usted es responsable del contenido que introduce en SpendSage, incluyendo gastos, notas, cuentas, facturas, reglas, nombres, correos, imágenes y descripciones de soporte.

SpendSage puede generar sugerencias, clasificaciones o resultados asistidos, incluyendo OCR o resúmenes. Esas salidas pueden contener errores, omisiones, coincidencias inexactas o inferencias incompletas. Usted debe revisar el contenido antes de confiar en él o actuar en consecuencia.

## 9. Escaneo, OCR, automatización y recomendaciones

Las funciones de escaneo, OCR, autocompletado, categorización automática, coaching o recomendaciones:

- se proporcionan con fines de asistencia;
- no sustituyen revisión humana;
- pueden variar según la calidad de la imagen, el contexto, el dispositivo, la configuración o la versión del producto;
- no constituyen asesoría profesional.

## 10. Suscripciones, compras y planes

Las pantallas de planes, facturación o restauración pueden representar funcionalidades actuales o preparatorias según la build.

Cuando la facturación real esté activa:

- las compras digitales en iOS se procesarán mediante los mecanismos autorizados de Apple;
- las suscripciones estarán sujetas además a las condiciones de App Store;
- la gestión, cancelación, renovación y reembolso podrán depender de Apple y de la normativa aplicable.

Mientras esas funciones no estén activas, la mera visualización de planes o estados no constituye una oferta vinculante de facturación en tiempo real.

## 11. Funciones familiares, compartidas o futuras

Ciertas referencias a planes familiares, colaboración o sincronización compartida pueden depender de activación técnica, plan aplicable, backend, elegibilidad de cuenta o configuración de tienda. La disponibilidad real puede variar por build, región o estado de despliegue.

## 12. Enlaces y servicios de terceros

SpendSage puede incluir enlaces o integraciones hacia:

- App Store o servicios de Apple;
- proveedores de autenticación;
- correo electrónico;
- documentación pública;
- sitios de soporte.

No controlamos de forma plena los servicios de terceros y no somos responsables de sus políticas, contenido, seguridad o disponibilidad.

## 13. Propiedad intelectual

La aplicación, sus personajes, diseño, arte, texto, marcas, interfaz, documentación y materiales relacionados pertenecen a SpendSage o a sus licenciantes y están protegidos por leyes de propiedad intelectual.

## 14. Exención de garantías

En la máxima medida permitida por la ley, SpendSage se proporciona "tal cual" y "según disponibilidad", sin garantías expresas o implícitas, incluyendo garantías de comerciabilidad, idoneidad para un propósito particular, no infracción, exactitud, disponibilidad continua o ausencia de errores.

## 15. Limitación de responsabilidad

En la máxima medida permitida por la ley, SpendSage y sus operadores, afiliados, licenciantes o colaboradores no serán responsables por daños indirectos, incidentales, especiales, ejemplares, punitivos o consecuenciales, ni por pérdida de beneficios, ahorros, datos, reputación o uso, derivados de:

- el uso o imposibilidad de uso de la aplicación;
- errores, sugerencias inexactas u omisiones;
- decisiones financieras adoptadas por el usuario;
- pérdida, divulgación o corrupción de datos exportados o compartidos por el usuario;
- interrupciones, fallos beta o cambios de funciones.

Nada en estos términos limita derechos irrenunciables del consumidor ni responsabilidades que no puedan excluirse conforme a la ley aplicable.

## 16. Indemnización

Usted acepta defender, indemnizar y mantener indemne a SpendSage y sus operadores frente a reclamaciones, pérdidas, daños, responsabilidades y costes razonables derivados de:

- su uso indebido de la aplicación;
- su incumplimiento de estos términos;
- contenido que usted introduzca o comparta en violación de derechos o leyes aplicables.

## 17. Suspensión o terminación

Podemos suspender, limitar o terminar el acceso a SpendSage, total o parcialmente, cuando:

- exista incumplimiento de estos términos;
- haya riesgo para la seguridad, integridad o legalidad del servicio;
- una build beta deba retirarse, sustituirse o desactivarse.

## 18. Ley aplicable y protección del consumidor

Estos términos se interpretarán conforme a la ley aplicable, sin perjuicio de las normas imperativas de protección al consumidor que correspondan en su jurisdicción.

## 19. Cambios a los términos

Podemos modificar estos términos para reflejar cambios legales, técnicos, comerciales o de producto. La fecha de "Última actualización" indicará la versión vigente.

## 20. Contacto

Para consultas legales, de soporte o del servicio, utilice los canales publicados por SpendSage en la aplicación, en la documentación pública o en el listado correspondiente de App Store/TestFlight.

Datos de identificación del operador actual:

- `Ruben Lazaro`
- RUC: `0924829179001`
- Domicilio declarado: `Samanes 4 Mz 408 Villa 130112`

Si posteriormente se publica un correo, razón social o canal formal adicional, este documento deberá actualizarse para reflejarlo de forma expresa.
