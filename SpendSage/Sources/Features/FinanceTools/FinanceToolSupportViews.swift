    ...
    var body: some View {
        HStack(alignment: .top, spacing: 14) {
            Image(systemName: systemImage)
                ...
                .background(BrandTheme.accent.opacity(0.2))
...
enum FinanceToolFormatting {
    ...
    static func paymentStatusText(for bill: BillRecord) -> String {
        guard let lastPaidAt = bill.lastPaidAt else {
            return bill.autopay ? "Autopay enabled" : "Awaiting payment"
        }
        return "Paid \(lastPaidAt.formatted(date: .abbreviated, time: .omitted))"
    }
}
